...
namespace lee8871_support {
	...
	public:
		...
		int setStream(std::ostream* record_ostream);
		int writeNum(const char* name, int value) override;
		int writeLongLongNum(const char * name, long long value)override;
		int writeU8Str(const char* name, srb::uint8  value) override;
		int writeU8ArrayStr(const char * name, const  srb::uint8  * data, int len)	 override;
		...
		int writeNumArray(const char * name, const  srb::uint8  * value, int len)override;
